...
epi_final_size<-function(r0,contact_matrix,demography_vector,prop_suscep=NULL){
  ...
  for(ii in 1:30){
    if(ii==1){
        ...
        new = xx+dx; new[new<0] = 1e-15 #f2 is only defined on values > 0
        iterate_output[ii,] = new
        xx = as.numeric(new)
      }
  }
  
  # print(sum( (1-iterate_output[iterations,])*demography_vector))
  
  if(sum( (1-iterate_output[iterations,])*demography_vector)< .02){ #If the overall attack rate isn't at least 1%, then consider the outbreak as not having happened
    return(rep(0, 100)) 
    }else{
