...
# set.seed(21)
# library(splines)
# dat.in = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$dth) & !is.na(dat.003$H1N1), ]
...
#   fits[[1]] = glm(dth ~ age, family = binomial, data = training.dat) # Linear fit
#   for(dd in 2:max.df){
#     fits[[dd]] = glm(dth ~ ns(age, df = dd), family = binomial, data = training.dat) # fits a nat spline with dd df
#   }
#   
#   
# 
...
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$dth, pch = '|', xlim = c(0, 100), xlab = 'age', ylab = 'prob prolonged symp.', main = paste('df =', jj), col = 'gray')
...
#   test.error.rate[ii, jj] = sum(abs(sim.outcomes - test.dat$dth))/nrow(test.dat)
# }
# }
```
...
# library(splines)
# ## Cross-validation template
# dat.in = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$dth) & !is.na(dat.003$H3N2), ]
# ##  Perform 10-fold cross-validation
# # 1. Partition the data into 10 folds
# fold.bounds = matrix(  c(0:9*floor(nrow(dat.in)/10)+1, 1:10*floor(nrow(dat.in)/10)+1), nrow = 10, ncol = 2); fold.bounds[10, 2] = nrow(dat.in) # Set indices at which folds start and end
# fold.inds = sample(1:nrow(dat.in), replace = FALSE) # Randomly order the observations in the data
...
#   plot.probs = predict(fits[[jj]], newdata = list(age = min(test.dat$age):max(test.dat$age)), type = 'response', se.fit = TRUE)
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$dth, pch = '|', xlim = c(0, 100), xlab = 'age', ylab = 'prob infected', main = paste('df =', jj), col = 'gray')
#   lines(min(test.dat$age):max(test.dat$age), plot.probs$fit, col = 'dodgerblue')
#   }
#   ## Continue cross validation
#   sim.outcomes = sapply(X = pred.probs, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
...
Conclusion: No clear support for a specific # of df. Visually, it looks like 3 df is enough to capture the overall shape with appropriate smoothness for both H1N1 and H3N2.
...
```{r echo = FALSE}
H1N1.model.dat = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$dth) & !is.na(dat.003$H1N1) & !is.na(dat.003$anyvac), ]
# Check that all relevant variables are not NA
which(is.na(H1N1.model.dat$dth))
...
H1N1.model.dat$country[which(H1N1.model.dat$country %in% c('Austria', 'Belgium', 'Chile', 'Norway', 'Peru', 'Poland', 'Singapore'))] = 'Other'
...
nrow(H1N1.model.dat)
test.inds = sample(1:nrow(H1N1.model.dat), size = 67, replace = FALSE)
...
fit0 = glm(dth ~ anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit0)

## H1N1 fit to age only, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit1 = glm(dth ~ ns(age, knots = 65) + anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit1)

## H1N1 fit to age plus imprinting, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit2 = glm(dth ~ ns(age, knots = 65) + p.g1.protection + anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit2)
...
pr0 = prediction(probs0, test.H1N1$dth)
pr1 = prediction(probs1, test.H1N1$dth)
pr2 = prediction(probs2, test.H1N1$dth)
...
master.AIC.table['003Death',  c('baseline', 'baseline+age', 'baseline+age+imp')] = AIC - min(AIC)
...
```{r echo = FALSE}
H3N2.model.dat = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$dth) & !is.na(dat.003$H3N2) & !is.na(dat.003$anyav), ]
# Check that all relevant variables are not NA
which(is.na(H3N2.model.dat$dth))
...
H3N2.model.dat$country[which(H3N2.model.dat$country %in% c('Belgium', 'China', 'Greece', 'Peru'))] = 'Other'
...
fit0 = glm(dth ~ anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit0)

## H3N2 fit to age only, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit1 = glm(dth ~ ns(age, knots = 65) + anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit1)

## H3N2 fit to age plus imprinting, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit2 = glm(dth ~ ns(age, knots = 65) + p.g2.protection + anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit2)
...
# Create a predict object
pr0 = prediction(probs0, test.H3N2$dth)
pr1 = prediction(probs1, test.H3N2$dth)
pr2 = prediction(probs2, test.H3N2$dth)
