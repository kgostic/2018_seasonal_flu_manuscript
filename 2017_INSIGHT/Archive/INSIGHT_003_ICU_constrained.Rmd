...
H1N1.model.dat$country[which(H1N1.model.dat$country %in% c('Belgium', 'Austria', 'Chile', 'China', 'Norway', 'Poland', 'Singapore'))] = 'Other'
...
H3N2.model.dat$country[which(H3N2.model.dat$country %in% c('Belgium', 'China', 'Germany', 'Greece', 'Peru', 'Singapore'))] = 'Other'
...
# Create a master protection column for H.group
...
# Create a master protection column for H.subtype
test$p.H.subtype = c(test.H1N1$p.H1.protection, test.H3N2$p.H3.protection)
train$p.H.subtype = c(train.H1N1$p.H1.protection, train.H3N2$p.H3.protection)

# Create a master protection column for N.subtype
test$p.N.subtype = c(test.H1N1$p.N1.protection, test.H3N2$p.N2.protection)
train$p.N.subtype= c(train.H1N1$p.N1.protection, train.H3N2$p.N2.protection)
...
fit3 = glm(anyicu ~ anyav + anydx + country + ns(age, knots = 65) + p.H.subtype + H3.valid*season + H3.valid*anyvac, data = train, family = binomial)
summary(fit3)

# Add imprinting at N.subtype level
fit4 = glm(anyicu ~ anyav + anydx + country + ns(age, knots = 65) + p.N.subtype + H3.valid*season + H3.valid*anyvac, data = train, family = binomial)
summary(fit4)
...
pr3 = prediction(probs3, test$anyicu)
pr4 = prediction(probs4, test$anyicu)
...
master.AIC.table['003ICU', c('baseline', 'baseline+age', 'baseline+age+imp', 'baseline+age+sub', 'baseline+age+N')] = AIC - min(AIC)
