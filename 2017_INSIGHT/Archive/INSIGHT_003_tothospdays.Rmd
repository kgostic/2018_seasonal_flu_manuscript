...
plot(jitter(valid$age), jitter(valid$tothospdays), ylim = c(1, 70), xlim = c(10, 100), xlab = 'age', ylab = 'symdur', col = 'dodgerblue', main = 'H1N1: Duration of hospitalization by age', cex = .7)
cor.test(valid$age, valid$tothospdays, method = 'spearman')
```
...
plot(jitter(valid$age), jitter(valid$tothospdays), ylim = c(0, 70), xlim = c(10, 100), xlab = 'age', ylab = 'symdur', col = 'firebrick1', main = 'H3N2: Duration of hospitalization by age', cex = .7)
cor.test(valid$age, valid$tothospdays, method = 'spearman')
```
...
```{r warning = FALSE, echo = FALSE}
valid = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$H1N1), ]
plot(factor(valid$country), jitter(valid$tothospdays),  xlab = '', ylab = 'symdur', col = 'lightblue', main = 'H1N1: Duration of hospitalization by country', cex = .7, las = 3)
```
...
```{r warning = FALSE, echo = FALSE}
valid = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$H3N2), ]
plot(factor(valid$country), jitter(valid$tothospdays),  xlab = '', ylim = c(0, 70), ylab = 'hosp days', col = 'pink3', main = 'H3N2: duration of hospitalization by country', cex = .7, las = 3)
```
...
```{r warning = FALSE, echo = FALSE}
valid = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$H1N1), ]
plot(factor(valid$season), jitter(valid$tothospdays),  xlab = '', ylab = 'hospital days', col = 'lightblue', main = 'H1N1: duration of hospitalization by season', cex = .7, las = 3)
```
...
```{r warning = FALSE, echo = FALSE}
valid = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$H3N2), ]
plot(factor(valid$season), jitter(valid$tothospdays),  xlab = '', ylim = c(0, 70), ylab = 'hosp days', col = 'pink3', main = 'H3N2: duraiton of hospitalization by season', cex = .7, las = 3)
```
...
#   fits[[1]] = glm(tothospdays ~ age, data = training.dat) # Linear fit
#   for(dd in 2:max.df){
#     fits[[dd]] = glm(tothospdays ~ ns(age, df = dd), data = training.dat) # fits a nat spline with dd df
#   }
#   
#   
# 
...
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$tothospdays, xlim = c(0, 100), xlab = 'age', ylab = 'Symptom duration', main = paste('df =', jj), col = 'dodgerblue')
...
#   MSE[ii, jj] = mean((preds - test.dat$tothospdays)^2, na.rm = TRUE)
# }
# }
```
...
```{r echo = FALSE}
H1N1.model.dat = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$H1N1) & !is.na(dat.003$tothospdays) & !is.na(dat.003$anyvac), ]
# Check that all relevant variables are not NA
which(is.na(H1N1.model.dat$tothospdays))
...
nrow(H1N1.model.dat)
test.inds = sample(1:nrow(H1N1.model.dat), size = 20, replace = FALSE)
...
```{r echo = FALSE}
H3N2.model.dat = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$H3N2) & !is.na(dat.003$tothospdays) & !is.na(dat.003$anyvac), ]
# Check that all relevant variables are not NA
which(is.na(H3N2.model.dat$tothospdays))
...
H3N2.model.dat$country[which(H3N2.model.dat$country %in% c('Belgium', 'China', 'Germany', 'Singapore', 'Spain'))] = 'Other'
...
# Split into training and test data
nrow(H3N2.model.dat)
test.inds = sample(1:nrow(H3N2.model.dat), size = 15, replace = FALSE)
