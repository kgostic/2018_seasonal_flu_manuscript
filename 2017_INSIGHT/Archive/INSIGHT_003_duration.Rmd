...
plot(jitter(valid$age), jitter(valid$symdur), ylim = c(1, 30), xlim = c(10, 100), xlab = 'age', ylab = 'symdur', col = 'dodgerblue', main = 'H1N1: Duration of symptoms by age', cex = .7)
...
#   fits[[1]] = glm(symdur ~ age, data = training.dat) # Linear fit
#   for(dd in 2:max.df){
#     fits[[dd]] = glm(symdur ~ ns(age, df = dd), data = training.dat) # fits a nat spline with dd df
#   }
#   
#   
# 
...
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$symdur, xlim = c(0, 100), xlab = 'age', ylab = 'Symptom duration', main = paste('df =', jj), col = 'dodgerblue')
...
#   MSE[ii, jj] = mean((preds - test.dat$symdur)^2, na.rm = TRUE)
# }
# }
```
...
```{r echo = FALSE}
# par(mfrow = c(1,1))
# plot(1:max.df, colMeans(MSE), ylab = 'test error rate', xlab = 'df', type = 'b', main = 'H1N1 10-fold CV to choose df')
```
...
```{r echo = FALSE}
H1N1.model.dat = dat.003[dat.003$H1N1 == 1 & !is.na(dat.003$H1N1) & !is.na(dat.003$symdur) & !is.na(dat.003$anyvac), ]
# Check that all relevant variables are not NA
which(is.na(H1N1.model.dat$symdur))
...
nrow(H1N1.model.dat)
test.inds = sample(1:nrow(H1N1.model.dat), size = 71, replace = FALSE)
...
```{r echo = FALSE}
H3N2.model.dat = dat.003[dat.003$H3N2 == 1 & !is.na(dat.003$H3N2) & !is.na(dat.003$symdur) & !is.na(dat.003$anyvac), ]
# Check that all relevant variables are not NA
which(is.na(H3N2.model.dat$symdur))
...
# Split into training and test data
nrow(H3N2.model.dat)
test.inds = sample(1:nrow(H3N2.model.dat), size = 74, replace = FALSE)
