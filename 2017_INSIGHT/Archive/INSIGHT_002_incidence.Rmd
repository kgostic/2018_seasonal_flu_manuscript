...
#barplot(rep(1, 3), col = cols, pch = 16)
```
...
valid = dat.002[!is.na(dat.002$H1N1), ]
tab = table(valid$age, valid$H1N1)
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
} )
plot(as.numeric(rownames(tab)), props, ylim = c(0, 1), xlim = c(10, 100), xlab = 'age', ylab = 'prop infected', col = 'lightblue', pch = 16, main = 'H1N1: Observed prop. infected by age')
...
```{r warning = FALSE, echo = FALSE}
valid = dat.002[!is.na(dat.002$H3N2), ]
tab = table(valid$age, valid$H3N2)
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
} )
plot(as.numeric(rownames(tab)), props, ylim = c(0, 1), xlim = c(10, 100), xlab = 'age', ylab = 'prop infected', col = 'pink3', pch = 16, main = 'H3N2: Observed prop. infected by age')
...
```{r warning = FALSE, echo = FALSE}
valid = dat.002[!is.na(dat.002$H1N1), ]
tab = table(valid$country, valid$H1N1)
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
...
ord = order(props)
plot(1:nrow(tab), props[ord], ylim = c(0, 1), xlab = '', ylab = 'prop infected', col = 'lightblue', pch = 16, main = 'H1N1: Observed prop. infected by country', xaxt = 'n')
...
```{r warning = FALSE, echo = FALSE}
valid = dat.002[!is.na(dat.002$H3N2), ]
tab = table(valid$country, valid$H3N2)
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
...
ord = order(props)
plot(1:nrow(tab), props[ord], ylim = c(0, 1), xlab = '', ylab = 'prop infected', col = 'pink3', pch = 16, main = 'H3N2: Observed prop. infected by country', xaxt = 'n')
axis(1, at = 1:nrow(tab), labels = rownames(tab)[ord], las = 3)
...
```{r warning = FALSE, echo = FALSE}
valid = dat.002[!is.na(dat.002$H1N1), ]
tab = table(valid$season, valid$H1N1)[-16, ]
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
...
#ord = order(props)
ord = 1:nrow(tab)
plot(1:nrow(tab), props[ord], ylim = c(0, 1), xlab = '', ylab = 'prop infected', col = 'lightblue', pch = 16, main = 'H1N1: Observed prop. infected by season', xaxt = 'n')
axis(1, at = 1:nrow(tab), labels = rownames(tab)[ord], las = 3)
points(1:nrow(tab), props[ord], col = 'dodgerblue')
segments(x0 = 1:nrow(tab), y0 = CIs[1,ord], y1 = CIs[2,ord], col = 'lightblue')
```
...
```{r warning = FALSE, echo = FALSE}
valid = dat.002[!is.na(dat.002$H3N2), ]
tab = table(valid$season, valid$H3N2)[-16, ]
props = tab[,2]/rowSums(tab)
CIs = sapply(X = 1:nrow(tab), FUN = function(ii){
pp = prop.test(x = tab[ii, 2], n = sum(tab[ii,]))
pp$conf.int[1:2]
...
#ord = order(props)
ord = 1:nrow(tab)
plot(1:nrow(tab), props[ord], ylim = c(0, 1), xlab = '', ylab = 'prop infected', col = 'pink3', pch = 16, main = 'H3N2: Observed prop. infected by age', xaxt = 'n')
axis(1, at = 1:nrow(tab), labels = rownames(tab)[ord], las = 3)
points(1:nrow(tab), props[ord], col = 'firebrick1')
segments(x0 = 1:nrow(tab), y0 = CIs[1,ord], y1 = CIs[2,ord], col = 'pink3')
```
...
# set.seed(23)
# library(splines)
# dat.in = dat.002[which(!is.na(dat.002$H1N1)), ]
...
#   fits[[1]] = glm(H1N1 ~ age, family = binomial, data = training.dat) # Linear fit
#   for(dd in 2:max.df){
#     fits[[dd]] = glm(H1N1 ~ ns(age, df = dd), family = binomial, data = training.dat) # fits a nat spline with dd df
#   }
#   
#   
# 
...
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$H1N1, pch = '|', xlim = c(0, 100), xlab = 'age', ylab = 'prob infected', main = paste('df =', jj), col = 'gray')
...
#   test.error.rate[ii, jj] = sum(abs(sim.outcomes - test.dat$H1N1))/nrow(test.dat)
# }
# }
```
...
# library(splines)
# ## Cross-validation template
# dat.in = dat.002[which(!is.na(dat.002$H3N2)), ]
# ##  Perform 10-fold cross-validation
# # 1. Partition the data into 10 folds
# fold.bounds = matrix(  c(0:9*floor(nrow(dat.in)/10)+1, 1:10*floor(nrow(dat.in)/10)+1), nrow = 10, ncol = 2); fold.bounds[10, 2] = nrow(dat.in) # Set indices at which folds start and end
# fold.inds = sample(1:nrow(dat.in), replace = FALSE) # Randomly order the observations in the data
...
#   ## Train the model
#   fits = vector('list', max.df); names(fits) = c('linear', paste('ns', 2:max.df, 'df', sep = ''))
#   fits[[1]] = glm(H3N2 ~ age, family = binomial, data = training.dat) # Linear fit
#   for(dd in 2:max.df){
#     fits[[dd]] = glm(H3N2 ~ ns(age, df = dd), family = binomial, data = training.dat) # fits a nat spline with dd df
#   }
#   
#   
# 
...
#   plot.probs = predict(fits[[jj]], newdata = list(age = min(test.dat$age):max(test.dat$age)), type = 'response', se.fit = TRUE)
#   ## Visualize results
#   plot(jitter(test.dat$age, factor = 2.5), test.dat$H3N2, pch = '|', xlim = c(0, 100), xlab = 'age', ylab = 'prob infected', main = paste('df =', jj), col = 'gray')
#   lines(min(test.dat$age):max(test.dat$age), plot.probs$fit, col = 'dodgerblue')
#   }
#   ## Continue cross validation
#   sim.outcomes = sapply(X = pred.probs, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
#   test.error.rate[ii, jj] = sum(abs(sim.outcomes - test.dat$H3N2))/nrow(test.dat)
# }
# }
```
...
```{r echo = FALSE}
H1N1.model.dat = dat.002[which(!is.na(dat.002$H1N1) & !is.na(dat.002$anyav)), ]

# Group countries with 30 or fewer valid cases into "other"
table(H1N1.model.dat$country)
H1N1.model.dat$country[which(H1N1.model.dat$country %in% c('Austria', 'Japan', 'Portugal'))] = 'Other'
...
fit0 = glm(H1N1 ~ anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit0)

## H1N1 fit to age only, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit1 = glm(H1N1 ~ ns(age, knots = 65) + anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit1)


## H1N1 fit to age plus imprinting, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit2 = glm(H1N1 ~ ns(age, knots = 65) + p.g1.protection + anyvac + anyav + anydx + country + season, data = train.H1N1, family = binomial)
summary(fit2)
...
pr0 = prediction(probs0, test.H1N1$H1N1)
pr1 = prediction(probs1, test.H1N1$H1N1)
pr2 = prediction(probs2, test.H1N1$H1N1)
...
# # Draw outcomes for each patient
# sim00 = sample(c(0,1), size = nrow(H1N1.model.dat), prob = c(.5, .5), replace = T)
# sim0 = sapply(X = pred0, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
# sim1 = sapply(X = pred1, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
# sim2 = sapply(X = pred2, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
# 
# # Estimate the simulated error rate
# error.rt = numeric(4); names(error.rt) = c('coin.flip', 'baseline', 'baeline+age', 'baseline+age+imp')
# error.rt[1] = sum(abs(sim00 - H1N1.model.dat$H1N1))/nrow(H1N1.model.dat)
# error.rt[2] = sum(abs(sim0 - H1N1.model.dat$H1N1))/nrow(H1N1.model.dat)
# error.rt[3] = sum(abs(sim1 - H1N1.model.dat$H1N1))/nrow(H1N1.model.dat)
# error.rt[4] = sum(abs(sim2 - H1N1.model.dat$H1N1))/nrow(H1N1.model.dat)
# sort(error.rt)
...
```{r echo = FALSE}
H3N2.model.dat = dat.002[which(!is.na(dat.002$H3N2) & !is.na(dat.002$anyav)), ]

# Group countries with 30 or fewer valid cases into "other"
table(H3N2.model.dat$country)
H3N2.model.dat$country[which(H3N2.model.dat$country %in% c('Austria', 'Chile', 'Japan', 'Portugal'))] = 'Other'
...
fit0 = glm(H3N2 ~ anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit0)

## H3N2 fit to age only, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit1 = glm(H3N2 ~ ns(age, knots = 65) + anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit1)

## H3N2 fit to age plus imprinting, plus fixed effects of vaccination, av use, underlying conditions, country and season
fit2 = glm(H3N2 ~ ns(age, knots = 65) + p.g2.protection + anyvac + anyav + anydx + country + season, data = train.H3N2, family = binomial)
summary(fit2)
...
# Create a predict object
pr0 = prediction(probs0, test.H3N2$H3N2)
pr1 = prediction(probs1, test.H3N2$H3N2)
pr2 = prediction(probs2, test.H3N2$H3N2)
...
# 
# # Draw outcomes for each patient
# sim00 = sample(c(0,1), size = nrow(H3N2.model.dat), prob = c(.5, .5), replace = T)
# sim0 = sapply(X = pred0, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
# sim1 = sapply(X = pred1, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
# sim2 = sapply(X = pred2, FUN = function(pp) sample(c(0,1), size = 1, prob = c(1-pp, pp)))
#   
# # Estimate the simulated error rate
# error.rt = numeric(4); names(error.rt) = c('coin.flip', 'baseline', 'baeline+age', 'baseline+age+imp')
# error.rt[1] = sum(abs(sim00 - H3N2.model.dat$H3N2))/nrow(H3N2.model.dat)
# error.rt[2] = sum(abs(sim0 - H3N2.model.dat$H3N2))/nrow(H3N2.model.dat)
# error.rt[3] = sum(abs(sim1 - H3N2.model.dat$H3N2))/nrow(H3N2.model.dat)
# error.rt[4] = sum(abs(sim2 - H3N2.model.dat$H3N2))/nrow(H3N2.model.dat)
# sort(error.rt)
...
# master.AIC.table = matrix(NA, ncol = 6, nrow = 7, dimnames = list(c('002Incidence', '002Prolonged', '003Symdur', '003Hospdays', '003ICU', '003Tothospdays', '003Death'), c('baseline', 'baseline+age', 'baseline+age+imp', 'simple.baseline', 'simple.baseline+age', 'simple.baseline+age+imp')))
# save(master.AIC.table, file = 'master.AIC.table.RData')
