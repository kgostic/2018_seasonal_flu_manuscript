...
############################################################################
###  Extract countires and seasons in which 90% 
##   or more of IAV circulation was caused by H1N1 
##   or H3N2, but not both
############################################################################
hist_H1fracs = melt(hist_H1fracs)
names(hist_H1fracs) = c('season', 'coutry', 'frac_H1_circ')
H1_seasons = hist_H1fracs[which(hist_H1fracs$frac_H1_circ > .9), ]
H1_seasons$cs = paste(H1_seasons$coutry, H1_seasons$season, sep = '.')
H3_seasons = hist_H1fracs[which(hist_H1fracs$frac_H1_circ < .1), ]
H3_seasons$cs = paste(H3_seasons$coutry, H3_seasons$season, sep = '.')

# Make a column to ID country and season combos
dat.002$cs = paste(dat.002$country, dat.002$season, sep = '.')
dat.002$mainsub = 'mixed' # Defaults value. Overwrite below where appropriate
dat.002$mainsub[dat.002$cs %in% H1_seasons$cs] = 'H1N1'
dat.002$mainsub[dat.002$cs %in% H3_seasons$cs] = 'H3N2'
# Drop all entries from seasons with mixed circulation
dat.002 = subset(dat.002, mainsub != 'mixed')
############################################################################
############################################################################
...
## Assume challenge is identical to dominant subtype
dat.002$challenge = dat.002$mainsub
...
pdf('INSIGHT_AgeDists_CleanedDatat.pdf')
...
xx = names(t2)
points(xx, t2/sum(t2), col = 'firebrick', pch = 16)
dev.off()
...
##  Protection status
####################
par(mfrow = c(3,2), bg = 'white', col.axis = 'black', col.lab = 'black', col.main = 'black', col = 'black', pch = 16)
...
pp = t(tab/rowSums(tab))[1,]
lo = log(pp/(1-pp))
plot(rowMeans(bins), lo, main = 'Protection status', xlab = 'p(protected)', ylab = '(log odds)_age', col = 'darkred')
...
library(mgcv)
m0 = gam(infected ~ age + anydx + anyav + anyvac*challenge + s(country, bs = 're') + s(season, bs ="re"), data = dat.002, family = binomial(link = "logit"), select = TRUE)
summary(m0)
...
m2 = gam(infected ~ s(protected_grp, by = challenge, bs = 'cr') + age + anydx + anyav + anyvac*challenge + s(country, bs = 're') + s(season, bs ="re"), data = dat.002, family = binomial(link = "logit"), select = TRUE)
summary(m2)
...
summary(m5)
par(mfrow = c(2,3))
plot.gam(m5, main = "Nonlinear, NA Imprinting")
par(mfrow = c(2,2))
gam.check(m5, type = 'pearson')

summary(m2)
par(mfrow = c(2,3))
plot.gam(m2, main = "Nonlinear, HA group Imprinting")
par(mfrow = c(2,2))
gam.check(m2, type = 'pearson')

AICsum = AIC(m0, m1, m2, m3, m4, m5, m6)
AICsum$del.AIC = AICsum$AIC - min(AICsum$AIC)
AICsum = AICsum[order(AICsum$del.AIC), ]
AICsum



## Linear imprinting only:
AIClinear = AIC(m0, m1, m4,m6)
AIClinear$del.AIC = AIClinear$AIC - min(AIClinear$AIC)
AIClinear = AIClinear[order(AIClinear$del.AIC), ]
AIClinear
