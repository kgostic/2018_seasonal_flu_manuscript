...
profile_function = function(pars, fixed.par.name, fixed.par.value, wAV, wDX, wVX, wPro.H1, dat.H1, wPro.H3, dat.H3, a18.24, a25.31, a32.38, a39.45, a46.52, a53.59, a60.66, a67.73, a74.80, a81.90, tested.master){
  # 1. Assign parameters to be fit
  rAV = ifelse(is.na(pars['rAV']), 1, pars['rAV']) # Relative risk given antiviral use
  rDX = ifelse(is.na(pars['rDX']), 1, pars['rDX']) # Relative risk given underlying symptoms
  rVX.H1 = ifelse(is.na(pars['rVX.H1']), 1, pars['rVX.H1']) # Relative risk given vaccination
  ...
  rAV = ifelse(fixed.par.name == 'rAV', fixed.par.value, rAV)
  rDX = ifelse(fixed.par.name == 'rDX', fixed.par.value, rDX)
  rVX.H1 = ifelse(fixed.par.name == 'rVX.H1', fixed.par.value, rVX.H1)
  rVX.H3 = ifelse(fixed.par.name == 'rVX.H3', fixed.par.value, rVX.H3)
  ...
  r81.90 = ifelse(fixed.par.name == 'r81.90', fixed.par.value, r81.90)
